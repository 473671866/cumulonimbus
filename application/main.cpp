...
unsigned __int64 global_var = 123456;
...
int main()
{
	...
	//bool success = GetApplicationModule(pid, "ntdll.dll", &module_address, nullptr);
	...
	//success = ReadMappingMemory(pid, (uint64_t)&global_var, &mapping, 8);
	//if (success) {
	...
	//success = ReadPhysicalMemory(pid, (uint64_t)&global_var, &physical, 8);
	//if (success) {
	...
	//success = WritePhysicalMemory(pid, (uint64_t)&global_var, &write, 8);
	//if (success) {
	//	std::cout << "写入成功\n";
	//}
	//else {
	...
	//*(unsigned __int64*)&x64buffer[12] = (uint64_t)test;
